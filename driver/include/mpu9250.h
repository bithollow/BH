...
public:
    ...
    */	
    bool init(int sample_rate_div = 1, int low_pass_filter = 0x06);

    /**
    * @brief set MPU9250 gyroscopes scale.
    * @param scale ranges : BITS_FS_250DPS BITS_FS_500DPS BITS_FS_1000DPS BITS_FS_2000DPS
    * @return the range set (250,500,1000 or 2000)
    */
    uint32_t setGyroScale(int scale);
    
    /**
    * @brief set MPU9250 accelerometer scale.
    * @param scale ranges : BITS_FS_2G BITS_FS_4G BITS_FS_8G BITS_FS_16G
    * @return the range set (2,4,8 or 16)
    */
    uint32_t setAccScale(int scale);
    ...
    */
    ...
    /**
    * @brief read gyposcope data.
    */
    void readGyro();

    /**
    * @brief get MPU9250 mag data.
    */
    void readMag();

    /**
    * @brief get MPU9250 accelerometer data, gypo and mag data.
    */
    void readAll();

    /**
    * @brief get MPU9250 address.
    * @return the I2C address (104)
    */
    uint32_t whoAmI();

    /**
    * @brief get MPU9250 AK8963 address.
    */
    ...
    */
    ...
    /**
    * @brief get MPU9250 accelerometer data, gypo and mag raw data.
    */
    void getMotionRaw9(int16_t *ax, int16_t *ay, int16_t *az,
                       int16_t *gx, int16_t *gy, int16_t *gz,
                       int16_t *mx, int16_t *my, int16_t *mz);
    /**
    * @brief get MPU9250 accelerometer data and gypo data.
    */					   
    void getMotion6(float *ax, float *ay, float *az, float *gx, float *gy, float *gz);

    /**
    * @brief get MPU9250 accelerometer raw data, gypo raw data.
    */
    void getMotionRaw6(int16_t *ax, int16_t *ay, int16_t *az,
                       int16_t *gx, int16_t *gy, int16_t *gz);
private:
    /**
    ...
    * @return write byte
    */
    uint32_t writeReg(uint8_t address, uint8_t data);

    /**
    * @brief read MPU9250 register.
    * @return read byte
    */
    uint32_t readReg(uint8_t address, uint8_t data);

    /**
    * @brief read MPU9250 registers.
    * @return read byte
    */
    void readRegs(uint8_t address, uint8_t *read_buffer, uint32_t buffer_length);
