...
int32_t UBXScanner::update(uint8_t data)
{
    if (state != Done) {
...
int32_t UBXParser::decodeMessage(std::vector<double>& data)
{
    int32_t flag=1;
    int32_t pos;
    uint16_t id;
...
int32_t UBXParser::checkMessage()
{
    int32_t flag=1;
    int32_t pos;
    uint8_t CK_A=0, CK_B=0;
...
int32_t Ublox::enableNAV_POSLLH()
{
    uint8_t gps_nav_posllh[] = {0xb5, 0x62, 0x06, 0x01, 0x03, 0x00, 0x01, 0x02, 0x01, 0x0E, 0x47};
    int32_t gps_nav_posllh_length = (sizeof(gps_nav_posllh)/sizeof(*gps_nav_posllh));
    uint8_t from_gps_data_nav[gps_nav_posllh_length];

    return SPI::transfer(spi_device_name.c_str(), gps_nav_posllh, from_gps_data_nav, gps_nav_posllh_length, 200000);
}

int32_t Ublox::enableNAV_STATUS()
{
    uint8_t gps_nav_status[] = {0xb5, 0x62, 0x06, 0x01, 0x03, 0x00, 0x01, 0x03, 0x01, 0x0F, 0x49};
    int32_t gps_nav_status_length = (sizeof(gps_nav_status)/sizeof(*gps_nav_status));
    uint8_t from_gps_data_nav[gps_nav_status_length];
...
int32_t Ublox::init()
{
    ...
    while (count < buffer_length/2) {
        ...
        SPI::transfer(spi_device_name.c_str(), &to_gps_data, &from_gps_data, 1, 200000);
...
int32_t Ublox::decodeMessages()
{
    int32_t status;
    uint8_t to_gps_data = 0x00;
    uint8_t from_gps_data = 0x00;
    std::vector<double> position_data;

    if (enableNAV_POSLLH() < 0) {
        printf("Could not configure ublox over SPI\n");
    }
...
int32_t Ublox::decodeSingleMessage(message_t msg, std::vector<double>& position_data)
{
    switch(msg) {
        case NAV_POSLLH:
            {
